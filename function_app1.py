...
def decode_jwt(token: str):
    try:
        ...
        keys = get_signing_keys(openid_config)

        # Use the first key to verify the JWT signature
        ...
        raise Exception("Invalid JWT signature or token.")
...
def validate_and_decode_token(req: func.HttpRequest):
    ...
    token = token.strip()  # Remove any leading/trailing spaces
    
    if not token.startswith("Bearer "):
        logging.error("Authorization header is malformed (does not start with 'Bearer').")
        raise Exception("Authorization header malformed.")
    
    try:
        # Remove "Bearer " prefix and decode the token
        token = token[7:]  # Directly remove 'Bearer ' part, no need to split
        logging.info(f"Decoding token: {token}")
        decoded_token = decode_jwt(token)  # Using the previously defined decode_jwt function
        logging.info(f"Token decoded successfully. Claims: {decoded_token}")
        return decoded_token
    except Exception as e:
        logging.error(f"Token validation failed. Error: {str(e)}")
        raise Exception("Unauthorized. Invalid or expired token.")

def decode_jwt(token: str):
    try:
        openid_config = get_openid_config()
        logging.debug(f"OpenID configuration: {openid_config}")
        
        keys = get_signing_keys(openid_config)
        logging.debug(f"Signing keys: {keys}")
        ...
        for key in keys["keys"]:
            try:
                ...
                logging.info(f"JWT signature verified, decoded token: {decoded_token}")
                return decoded_token
            except InvalidTokenError as e:
                logging.debug(f"Invalid token for this key: {e}")
                continue

        raise Exception("Invalid JWT signature or token.")

    except Exception as e:
        logging.error(f"Failed to decode JWT: {str(e)}")
        raise Exception(f"Failed to decode JWT: {str(e)}")
...
def get_employee(req: func.HttpRequest) -> func.HttpResponse:
    logging.info(f"Received request for employee details. Headers: {req.headers}")
